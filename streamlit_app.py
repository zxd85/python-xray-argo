...
async def generate_links(argo_domain):
    ...
    try:
        # 运行 curl 命令，确保 capture_output 和 text=True
        ...
        if meta_info.returncode == 0 and meta_info.stdout:
            try:
                # 安全地解析 JSON 数据
                data = json.loads(meta_info.stdout)
                
                # 安全获取 country 和 isp，使用 .get() 方法
                country = data.get('country', 'XX')
                isp_name = data.get('isp', 'Unknown')
                
                # 拼接 ISP 字符串，进行清理
                ISP = f"{country}-{isp_name}".replace(' ', '_').strip()
            except json.JSONDecodeError:
                print("Cloudflare Meta JSON 解析失败，使用默认 ISP 名称。")
        else:
            print("Cloudflare Meta 网络请求失败，使用默认 ISP 名称。")
            
    except Exception as e:
        print(f"获取 ISP 信息发生异常: {e}，使用默认 ISP 名称。")
    # --- 修复结束 ---
    
    time.sleep(2)
    # 使用修复后的 ISP 变量继续执行原有的代码逻辑
